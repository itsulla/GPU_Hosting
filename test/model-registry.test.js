...
const HTTPS_FIRST_PARTY = /^https:\/\/(?:www\.)?(?:deepseek\.com|huggingface\.co|qwenlm\.github\.io|github\.com\/(?:QwenLM|deepseek-ai|moonshotai|MiniMax-AI|google|openai|meta-llama|mistralai|sarvamai)|ai\.google\.dev|ai\.meta\.com|mistral\.ai|docs\.mistral\.ai|docs\.sarvam\.ai|platform\.anthropic\.com|docs\.anthropic\.com|platform\.moonshot\.cn|www\.alibabacloud\.com)/i;
...
test('every record has complete, dated, first-party evidence metadata', () => {
  ...
  for (const record of records) {
    for (const field of ['slug', 'displayName', 'vendor', 'category', 'availability', 'weightsStatus', 'license', 'bestFor', 'facts']) assert.ok(field in record, `${field} missing`);
    ...
    assert.ok(['verified', 'pending'].includes(record.evidenceStatus));
...
test('required current self-hostable models carry the supplied verified facts', () => {
  ...
  const expected = {
    ...
    'qwen3.6-27b': [27.8, null, 262144, 'Apache-2.0'],
    ...
    'google-gemma-4': [27, null, 128000, 'Gemma license'],
    ...
    'llama-4-maverick': [402, 17, 1000000, 'Llama 4 Community License'],
    'mistral-small-4': [119, 6, 262144, 'Apache-2.0'],
    'mistral-large-3': [675, 41, 262144, 'Apache-2.0'],
    'sarvam-105b': [106, 10.3, 128000, 'Apache-2.0'],
  };
...
test('API comparators preserve exact identifiers and never become deployable', () => {
  ...
  for (const [slug, apiId] of Object.entries({
    'claude-fable-5': 'claude-fable-5-20260726',
    'claude-opus-5': 'claude-opus-5-20260726',
    'claude-sonnet-5': 'claude-sonnet-5-20260726',
    'kimi-k3-api': 'kimi-k3',
